 ...
 * @cdev: pointer to this cooling device, after thermal registration
...
static int tegra124_dfll_cdev_get_max_state(struct thermal_cooling_device *cdev,
...
static int tegra124_dfll_cdev_get_cur_state(struct thermal_cooling_device *cdev,
...
static int tegra124_dfll_cdev_set_state(struct thermal_cooling_device *cdev,
					unsigned long cur_state)
{
	struct platform_device *pdev = (struct platform_device *)cdev->devdata;
...
static struct thermal_cooling_device_ops tegra124_dfll_cooling_ops = {
	.get_max_state = tegra124_dfll_cdev_get_max_state,
	.get_cur_state = tegra124_dfll_cdev_get_cur_state,
	.set_cur_state = tegra124_dfll_cdev_set_state,
};
...
static int tegra124_dfll_cdev_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(cdev_type)) {
		dev_err(dev,
			"Tegra124 DFLL thermal reaction: missing cdev type\n");
		ret = -EINVAL;
		goto error;
	}
	tcd = thermal_cooling_device_register((char *)cdev_type,
					      (void *)dfll_pdev,
					      &tegra124_dfll_cooling_ops);
	if (IS_ERR(tcd)) {
		dev_err(dev,
	...
	tegra124_dfll_cdev_data.cdev = tcd;
	...
	dev_info(dev, "Tegra124 DFLL 'cooling device' registered\n");
...
static int tegra124_dfll_cdev_remove(struct platform_device *pdev)
{
	tegra124_dfll_detach_thermal(tegra124_dfll_cdev_data.dfll_pdev,
				     tegra124_dfll_cdev_data.cdev);
	thermal_cooling_device_unregister(tegra124_dfll_cdev_data.cdev);
