			...
			unsigned long *parent_rate)
{
	struct tegra_clk_cbus_shared *cbus = to_clk_cbus_shared(hw);
	unsigned long *freqs;
	int ret, i, num_freqs = 0;

	ret = tegra_dvfs_get_freqs(hw->clk, &freqs, &num_freqs);
	if (ret < 0 || !num_freqs) {
		struct clk *parent;
		long new_rate;
	...
	if (!cbus->min_rate) {
		cbus->min_rate = freqs[0];
		WARN_ON(!cbus->min_rate);
	}

	rate = max(rate, cbus->min_rate);
		...
		unsigned long max_rate)
{
	...
	return clk_register(NULL, &system->hw);
}
		...
		unsigned long min_rate, unsigned long max_rate)
{
	...
	return clk_register(NULL, &master->hw);
}
