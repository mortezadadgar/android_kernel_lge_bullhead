...
#define DUMP_REG(a) seq_printf(m, "%-32s\t%03x\t%08lx\n",		\
...
static int tegra_dc_hdmi_init(struct tegra_dc *dc)
{
	...
	res = platform_get_resource_byname(dc->ndev, IORESOURCE_MEM, "hdmi");
	if (!res) {
	...
	disp1_clk = clk_get(&dc->ndev->dev, "disp1");
	...
	disp2_clk = clk_get(&dc->ndev->dev, "disp2");
...
static void tegra_dc_hdmi_setup_eld_buff(struct tegra_dc *dc)
{
	...
	for (j = 0; j < hdmi->eld.mnl; j++) {
		tegra_hdmi_writel(hdmi,
			((j + HDMI_ELD_MONITOR_NAME_INDEX) << 8) |
			 (hdmi->eld.monitor_name[j]),
			  HDMI_NV_PDISP_SOR_AUDIO_HDA_ELD_BUFWR_0);
	}
	for (j = 0; j < hdmi->eld.sad_count; j++) {
		tegra_hdmi_writel(hdmi,
		      ((j + HDMI_ELD_MONITOR_NAME_INDEX + hdmi->eld.mnl) << 8) |
			(hdmi->eld.sad[j]),
			  HDMI_NV_PDISP_SOR_AUDIO_HDA_ELD_BUFWR_0);
	}
					...
					unsigned audio_source)
{
	...
	if (tegra_hdmi_calc_audio_cts_n(audio_freq, clk_get_rate(hdmi->clk),
	...
	tegra_hdmi_writel(hdmi, ACR_SUBPACK_CTS(hda_cts),
	...
	tegra_hdmi_writel(hdmi, hda_aval, reg_addr);
				       ...
				       unsigned audio_source)
{
	...
	if (AUDIO_FREQ_32K == audio_freq ||
		...
		if (hdmi->clk_enabled)
			tegra_dc_hdmi_setup_audio(hdmi->dc,
						  audio_freq, audio_source);
...
static void tegra_dc_hdmi_enable(struct tegra_dc *dc)
{
	...
	tegra_dc_writel(dc,
			PULSE_START(pulse_start) | PULSE_END(pulse_start + 8),
...
bool tegra_dc_hdmi_mode_change(const struct tegra_dc *dc)
{
	struct tegra_dc_hdmi_data *hdmi = tegra_dc_get_outdata(dc);
	tegra_dc_setup_clk((struct tegra_dc *)dc, hdmi->clk);
	clk_set_rate(hdmi->clk, dc->mode.pclk);
	return true;
}
