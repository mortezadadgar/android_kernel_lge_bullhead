...
static const struct of_device_id pmc114_match[] = {
	{ .compatible = "nvidia,tegra114-pmc" },
	{},
};

static const struct of_device_id pmc124_match[] = {
	...
	const struct tegra_dc_dp_link_config *cfg)
{
	...
	struct clk			*clk = NULL;
	struct clk			*clk_safe = NULL;
	struct clk			*clk_edp = NULL;
	int				 err;
	...
	/* Get pmc_base cause we will read/write it when setting pad dpd */
	if (is_tegra114())
		node = of_find_matching_node(NULL, pmc114_match);
	if (is_tegra124())
		node = of_find_matching_node(NULL, pmc124_match);
	...
	clk_safe = clk_get(&dc->ndev->dev, "clk_m");
	if (IS_ERR_OR_NULL(clk_safe)) {
		dev_err(&dc->ndev->dev, "sor: get sor safe clk failed.\n");
		err = -ENOENT;
		goto err_put_clk;
	}

	clk_edp = clk_get(&dc->ndev->dev, "pll_dp");
	if (IS_ERR_OR_NULL(clk_edp)) {
		dev_err(&dc->ndev->dev, "sor: get sor edp clk failed.\n");
		err = -ENOENT;
		goto err_put_clk;
	}
	...
	sor->dc	      = dc;
	sor->base     = base;
	sor->pmc_base = pmc_base;
	sor->base_res = base_res;
	sor->sor_clk  = clk;
	sor->sor_clk_safe = clk_safe;
	sor->sor_clk_edp = clk_edp;
	sor->link_cfg = cfg;
	sor->portnum  = 0;
...
{
	clk_put(sor->sor_clk_edp);
	clk_put(sor->sor_clk_safe);
	clk_put(sor->sor_clk);
	iounmap(sor->pmc_base);
	iounmap(sor->base);
	...
	const struct tegra_dc_dp_link_config *cfg)
{
	...
	tegra_dc_sor_set_dp_linkctl(sor, true, training_pattern_none, cfg);
...
{
	u32 reg_val;
	void __iomem *pmc_base = sor->pmc_base;
...
static void tegra_sor_enable_safe_clock(struct tegra_dc_sor_data *sor)
{
	int ret;

	ret = clk_set_parent(sor->sor_clk, sor->sor_clk_safe);
	WARN(ret, "Enable SOR safe clock failed: %d\n", ret);
}
...
static void tegra_sor_enable_edp_clock(struct tegra_dc_sor_data *sor)
{
	...
	ret = clk_set_parent(sor->sor_clk, sor->sor_clk_edp);
	WARN(ret, "Enable SOR edp clock failed: %d\n", ret);
}
				  ...
				  bool is_lvds)
{
	...
	/* Enable safe clock */
	tegra_sor_enable_safe_clock(sor);
...
{
	if (!__clk_get_enable_count(sor->sor_clk))
		clk_prepare_enable(sor->sor_clk);
}
...
void tegra_dc_sor_enable_dp(struct tegra_dc_sor_data *sor)
{
	...
	/* re-enable SOR clock */
	tegra_sor_enable_edp_clock(sor);
