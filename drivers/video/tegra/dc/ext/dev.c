...
static void tegra_dc_ext_flip_worker(struct work_struct *work)
{
	...
	struct tegra_dc_win **wins;
	struct tegra_dc_dmabuf **unpin_handles;
	...
	for (i = 0; i < win_num; i++) {
		...
		list_for_each_entry(temp, &ext_win->timestamp_queue,
			...
			struct timespec *ts;
			if (j == 0) {
				ts = &flip_win->attr.timestamp;
				if (unlikely(temp != data))
					dev_err(&win->dc->ndev->dev,
						"work queue did NOT dequeue"
						" head!!!");
				else
					head_timestamp = timespec_to_ns(ts);
			} else {
				s64 timestamp;

				ts = &temp->win[i].attr.timestamp;
				timestamp = timespec_to_ns(ts);

				skip_flip =
					!tegra_dc_does_vsync_separate(ext->dc,
				 ...
				 __u32 *syncpt_id, __u32 *syncpt_val)
{
	...
	if (has_timestamp) {
		...
		list_add_tail(&data->timestamp_node,
			      &ext->win[work_index].timestamp_queue);
				   ...
				   struct tegra_dc_ext_feature *feature)
{
	...
	if (dc->enabled && feature->entries) {
		...
		if (copy_to_user(feature->entries, table->entries,
				 table->num_entries *
...
int __init tegra_dc_ext_module_init(void)
{
	...
	if (!tegra_dc_ext_class) {
		pr_err("tegra_dc_ext: failed to create class\n");
		return -ENOMEM;
	}
