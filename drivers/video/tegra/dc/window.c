		      ...
		      u32 *flags, int win_num)
{
	...
	if (win_num != 3) {
		...
		if (!infront_mask) {
			...
			/* If the first (top) window is not opaque, check if
			 * the current (middle) window is not opaque. If yes
			 * then we need to enable appropriate blending mode
			 * otherwise current window is dependent on the first
			 * window. If the first window is opaque then the
			 * current window has no contribution and we set its
			 * weight to 0.
			 */
			...
			/* If both windows above the bottom window are not
			 * opaque then the bottom window is dependent otherwise
			 * the bottom window has no contribution and we set
			 * its weight to 0.
			 */
...
int tegra_dc_sync_windows(struct tegra_dc_win *windows[], int n)
{
	...
	ret = wait_event_interruptible_timeout(windows[0]->dc->wq,
			tegra_dc_windows_are_clean(windows, n), HZ);
