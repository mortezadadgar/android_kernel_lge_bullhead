...
static inline void *tegra_dc_get_outdata(const struct tegra_dc *dc)
{
	return dc->out_data;
}
...
static inline void tegra_dc_powergate_locked(struct tegra_dc *dc)
{
	...
	if (tegra_powergate_is_powered(dc->powergate_id)) {
		ret = tegra_powergate_partition(dc->powergate_id);
		if (ret < 0)
			dev_err(&dc->ndev->dev, "%s: could not powergate: %d\n",
					__func__, ret);
	}
}
