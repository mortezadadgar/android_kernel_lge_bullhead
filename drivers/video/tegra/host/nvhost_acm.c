...
int nvhost_module_init(struct platform_device *dev)
{
	...
	struct nvhost_device_data *pdata = platform_get_drvdata(dev);
	struct kobj_attribute *power_attr;
	...
	while (i < NVHOST_MODULE_MAX_CLOCKS && pdata->clocks[i].name) {
		...
		snprintf(devname, MAX_DEVID_LENGTH,
			 ...
			 dev->name, dev->id);
		c = clk_get(&dev->dev, pdata->clocks[i].name);
		if (IS_ERR(c)) {
	...
	power_attr = pdata->power_attrib->power_attr;

	attr = &power_attr[NVHOST_POWER_SYSFS_ATTRIB_CLOCKGATE_DELAY];
	...
	attr = &power_attr[NVHOST_POWER_SYSFS_ATTRIB_POWERGATE_DELAY];
