			      ...
			      u32 flags)
{
	...
	unsigned long old_rate = gk20a_clk_get_rate(g);

	if (rounded_rate == old_rate) {
		*freq = rounded_rate;
		return 0;
	}

	if (rounded_rate > old_rate)
		gk20a_dvfs_adjust_voltage(g, rounded_rate);
...
void nvhost_gk20a_scale_init(struct platform_device *pdev)
{
	...
	if (gk20a_dvfs_init(pdev, g)) {
		dev_warn(&pdev->dev, "failed to initialize gk20a dvfs\n");
		goto err_init_dvfs;
	}
	...
	profile->ext_stat.min_freq = gk20a_dvfs_get_min_freq(g);
	profile->ext_stat.max_freq = gk20a_dvfs_get_max_freq(g);
