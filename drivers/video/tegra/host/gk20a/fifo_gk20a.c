...
static int init_engine_info(struct fifo_gk20a *f)
{
	...
	for (i = 0; i < max_info_entries; i++) {
		...
		if (entry == top_device_info_entry_enum_v() &&
			...
			nvhost_dbg_info("gr info: runlist_id %d",
				gr_info->runlist_id);

			gr_info->engine_id =
				top_device_info_engine_enum_v(table_entry);
			nvhost_dbg_info("gr info: engine_id %d",
				gr_info->engine_id);

			runlist_bit = 1 << gr_info->runlist_id;

			for (pbdma_id = 0;
			     pbdma_id < f->num_pbdma;
			     pbdma_id++) {
...
int gk20a_init_fifo_reset_enable_hw(struct gk20a *g)
{
	...
	/* TBD: alternative intr tree*/
	gk20a_writel(g, fifo_intr_en_0_r(), 0xFFFFFFFF);
	/* TBD: alternative intr tree*/
	gk20a_writel(g, fifo_intr_en_1_r(), 0xFFFFFFFF);
...
static void gk20a_fifo_handle_mmu_fault(struct gk20a *g)
{
	...
	for_each_set_bit(engine_mmu_id, &fault_id, 32) {
		...
		if (ch) {
			...
			if (gk20a_fifo_should_defer_engine_reset(
					g, engine_id, &f, fake_fault)) {
...
static u32 fifo_error_isr(struct gk20a *g, u32 fifo_intr)
{
	...
	reset_channel = !g->fifo.deferred_reset_pending &&
			(reset_channel || fifo_intr);
					...
					u32 pbdma_id)
{
	...
	if (pbdma_intr_0) {
		...
		if (f->intr.pbdma.channel_fatal_0 & pbdma_intr_0) {
			dev_warn(dev, "channel error: pbdma_intr_0(%d):0x%08x",
				 pbdma_id, pbdma_intr_0);
			reset_channel = true;
