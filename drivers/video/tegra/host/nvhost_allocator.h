...
struct nvhost_allocator {
	...
	/*
	 * first free addr, non-contigous allocation preferred start,
	 * in order to pick up small holes
	 */
	u32 first_free_addr;

	/* last free addr, contiguous allocation preferred start */
	u32 last_free_addr;

	/* max free hole size up to last_free_addr */
	u32 cached_hole_size;
...
struct nvhost_alloc_block {
	...
	/* linear space range [start, end) */
	u32 start;
	u32 end;

	/*
	 * backing structure for this linear space block page table,
	 * comp tag, etc
	 */
	void *priv;

	/* prev block with lower address */
	struct nvhost_alloc_block *prev;
	/* next block with higher address */
	struct nvhost_alloc_block *next;

	bool nc_block;
	/* prev block for non-contiguous allocation */
	struct nvhost_alloc_block *nc_prev;
	/* next block for non-contiguous allocation */
	struct nvhost_alloc_block *nc_next;
};
