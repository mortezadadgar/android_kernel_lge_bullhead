...
static struct sysfs_dirent *low_mem_available_dirent;
...
{
	struct sysfs_dirent *low_mem_node;
	int err = sysfs_create_group(mm_kobj, &low_mem_attr_group);
	if (err)
		pr_err("low_mem: register sysfs failed\n");

	low_mem_node = sysfs_get_dirent(mm_kobj->sd, NULL, "chromeos-low_mem");
	if (low_mem_node) {
		low_mem_available_dirent =
		    sysfs_get_dirent(low_mem_node, NULL, "available");
