...
TRACE_EVENT(nvhost_channel_write_submit,
	...
	TP_printk("name=%s, count=%d, cmdbufs=%u, relocs=%u, "
...
TRACE_EVENT(nvhost_ioctl_channel_submit,
	...
	TP_printk("name=%s, version=%u, cmdbufs=%u, relocs=%u, "
		  "waitchks=%u, syncpt_id=%u, syncpt_incrs=%u",
...
TRACE_EVENT(nvhost_channel_write_reloc,
	...
	TP_printk("name=%s, cmdbuf_mem=%08x, cmdbuf_offset=%04x, "
		  "target=%08x, target_offset=%04x",
...
TRACE_EVENT(nvhost_ioctl_channel_set_ctxswitch,
	...
	TP_printk("name=%s, ctx=%p, save_mem=%08x, save_offset=%d, "
		  "save_words=%d, restore_mem=%08x, restore_offset=%d, "
		  "restore_words=%d, syncpt_id=%d, waitbase=%d, "
		  "save_incrs=%d, restore_incrs=%d",
	  __entry->name, __entry->ctx,
...
TRACE_EVENT(nvhost_gk20a_mmu_fault,
	    ...
	    TP_printk("fault=0x%x,%08x info=0x%x instance=0x%x engine_id=%d "
		      "engine=%s client=%s type=%s",
		      __entry->fault_hi, __entry->fault_lo,
		      __entry->fault_info, __entry->instance,
		      __entry->engine_id, __entry->engine,
		      __entry->client, __entry->fault_type)
);
