...
#define MAX98090_BASE_ADDRESS 0x10
#define ASOC_BIN_PLATFORM_DRIVER "sst-platform"
#define ASOC_MACHINE_DRIVER "byt_max98090"
...
static const struct i2c_board_info i2c_bus_info[] = {
	...
	{ I2C_BOARD_INFO("max98090", MAX98090_BASE_ADDRESS) },
...
void *byt_audio_platform_data(void *info)
{
	...
	adap = i2c_get_adapter(CONFIG_SND_BYT_MAX98090_I2C_BUSNUM);
	if (!adap) {
		pr_err("BYT : i2c_get_adapter(%d) failed\n",
				CONFIG_SND_BYT_MAX98090_I2C_BUSNUM);
		return NULL;
	}
	client = i2c_new_device(adap, i2c_bus_info);
	if (!client) {
		pr_err("BYT : i2c_new_device failed\n");
		return NULL;
	}
