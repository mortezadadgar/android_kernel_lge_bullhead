...
static int tegra_core_dvfs_clk_event(struct notifier_block *this,
...
static struct notifier_block tegra_core_dvfs_nb = {
	.notifier_call = tegra_core_dvfs_clk_event,
	.priority = 1,
};
...
static int tegra_config_dvfs(struct dvfs_rail *rail)
{
	...
	list_for_each_entry(d, &rail->dvfs, reg_node) {
		...
		if (rail == tegra_core_rail)
			clk_notifier_register(d->clk, &tegra_core_dvfs_nb);
	}
...
static int tegra_dvfs_remove(struct platform_device *pdev)
{
	...
	list_for_each_entry(d, &tegra_core_rail->dvfs, reg_node) {
		clk_notifier_unregister(d->clk, &tegra_core_dvfs_nb);
	}
