			      ...
			      bool with_check)
{
	...
	if (substream->runtime &&
		...
		if (delay >= azx_dev->period_bytes) {
			dev_warn(chip->card->dev, "Unstable LPIB (%d >= %d); "
...
static int azx_reset(struct azx *chip, int full_reset)
{
	...
	if (!azx_readb(chip, GCTL)) {
		dev_info(chip->card->dev, "azx_reset: controller not ready!\n");
		return -EBUSY;
	}
	...
	if (!chip->codec_mask) {
		...
		dev_info(chip->card->dev, "codec_mask = 0x%x\n",
			 chip->codec_mask);
	}
...
irqreturn_t azx_interrupt(int irq, void *dev_id)
{
	...
	if (chip->driver_caps & AZX_DCAPS_PM_RUNTIME &&
	    chip->card->dev->power.runtime_status != RPM_ACTIVE)
		return IRQ_NONE;
#endif
		     ...
		     int *power_save_to)
{
	...
	if (chip->driver_caps & AZX_DCAPS_RIRB_DELAY) {
		dev_info(chip->card->dev, "Enable delay in RIRB handling\n");
